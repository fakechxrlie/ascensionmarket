...
export default function Verify() {
  ...
  // Step 2: KYC requirements (stored as Base64 data URLs)
  ...
  const handleNextStep = () => {
    if (!dob || !country || !stateRegion) {
      setErrorMsg('Please fill out all personal details before proceeding.');
      return;
    }
    setErrorMsg('');
    setStep(2);
  ...
  const handleVerify = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/applications', {
        ...
        body: JSON.stringify({
          kycData: JSON.stringify({ doc1, doc2, dob, country, stateRegion }),
          games: selectedGames,
      ...
      <div className="panel" style={{ maxWidth: '650px', width: '100%', marginTop: '30px', border: '1px solid var(--border-light)', background: 'var(--bg-card)', padding: '24px' }}>
        <div style={{ borderBottom: '1px solid var(--border-light)', paddingBottom: '12px', marginBottom: '20px' }}>
          <h2 className="font-mono" style={{ fontSize: '1.25rem', color: 'var(--brand)', margin: 0 }}>
            // BOOSTER APPLICATION - STEP {step} / 2
          </h2>
          <span className="font-mono" style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>
            STATUS: {step === 1 ? 'PERSONAL DETAILS' : 'CREDENTIALS & GAMES'}
          </span>
        </div>
        ...
        <p style={{ color: 'var(--text-muted)', marginBottom: '20px', fontSize: '0.85rem', lineHeight: '1.4' }}>
          Verification is required to become a booster on Ascension. Please complete all fields. <strong>All files are secure and use end-to-end encryption.</strong>
        </p>

        {step === 1 && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            <div className="form-group" style={{ marginBottom: 0 }}>
              <label className="font-mono" style={{ fontSize: '0.75rem' }}>DATE OF BIRTH</label>
              <input type="date" required className="input-field" value={dob} onChange={e => setDob(e.target.value)} />
            </div>
            <div className="form-group" style={{ marginBottom: 0 }}>
              <label className="font-mono" style={{ fontSize: '0.75rem' }}>COUNTRY OF RESIDENCE</label>
              <input type="text" required className="input-field" placeholder="e.g. United States" value={country} onChange={e => setCountry(e.target.value)} />
            </div>
            <div className="form-group" style={{ marginBottom: 0 }}>
              <label className="font-mono" style={{ fontSize: '0.75rem' }}>STATE / REGION / PROVINCE</label>
              <input type="text" required className="input-field" placeholder="e.g. California" value={stateRegion} onChange={e => setStateRegion(e.target.value)} />
            </div>
            
            <button type="button" onClick={handleNextStep} className="btn-primary" style={{ marginTop: '10px' }}>
              NEXT STEP ➜
            </button>
          </div>
        )}
        ...
        {step === 2 && (
          <form onSubmit={handleVerify}>
            ...
            <div style={{ display: 'flex', gap: '10px' }}>
              <button type="button" onClick={() => setStep(1)} className="btn-secondary" style={{ flex: 1, borderColor: 'var(--border-light)', color: 'var(--text-muted)' }}>
                🠔 BACK
              </button>
